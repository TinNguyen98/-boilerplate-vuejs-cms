<template>
  <a-layout-sider
    ...
    :style="{ overflow: 'auto', height: 'calc(100vh - 48px)', position: 'fixed', left: 0, zIndex: 10}">

    <a-menu mode="inline">
      <template v-for="sidebar in SIDEBAR">
        <template v-if="validProfile && sidebar.dropdown && sidebar.role.includes(userProfile.role)">
          <a-sub-menu :key="sidebar.id" style="padding-left: 16px">
            <span slot="title" style="padding-left: 0">
              <a-icon :type="sidebar.icon"/>
              <span v-text="$t(sidebar.title)"/>
            </span>

            <template v-for="children in sidebar.dropdownItems">
              <router-link v-if="children.role.includes(userProfile.role)"
        ...
        <router-link v-else-if="validProfile && sidebar.role.includes(userProfile.role)"
                     :key="sidebar.id"
          ...
          <li @click="navigate"
              role="menuitem"
              :class="{ 'ant-menu-item-selected': isActive || sidebar.childrenRoutes.includes($route.name) }">
            <a-icon :type="sidebar.icon"/>
            <span :title="$t(sidebar.title)"
