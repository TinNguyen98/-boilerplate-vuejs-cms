  ...
  <ValidationObserver
    ...
    <div class="main-form_container pb-4">
      ...
      <div class="main-form_row d-flex flex-wrap pb-5">
        <!-- Effect movie frame -->
        <InputSelect
          v-model="form.effect_movie_frame"
      ...
      <div
        ...
        <div
          ...
          <a-checkbox
            class="checkbox-animated_checkbox"
            :class="{ checked: isPriority }"
            :disabled="recordDetail.status === 'upcoming'"
            v-model="isPriority" />
          <span
            class="checkbox-animated_desc"
            :class="{ 'turn-on': isPriority }"
            v-text="
              $t(`management_event.${isPriority ? 'open' : 'close'}_priority`)
            " />
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    fillOptionsByType(type) {
      ...
      const listDependent =
        type === 'collection' ? this.collectionList : this.frameList
